...
# 版本：OpenHarmony 6.1不断提升北向应用开发者体验，增强端云通信，基础语音，预览类等基础常用API和能力；增强的系统应用的能力，包括文件管理，图库，设置，相机，通话等。 
## 发布日期：2026年H1 
### 版本概述
* **新增支持P7885芯片开发板	新增如下能力：**
 
	* **持5G蜂窝通信能力**，提供驻网、通话、短信、数据功能。支持Vulkan/OpenGL统一渲染。
	* **支持GNSS 卫星状态信息上报**，可识别并上报 GPS、北斗、GLONASS 等卫星数据。

	* **适配星闪驱动**，支持星闪SLE 1.0，支持星闪配对连接和数据传输。

	* **板载适配6类传感器**：加速度计、陀螺仪、磁力计、接近传感器、环境光传感器、马达。

	* **板载36pin标准PCIE接口**，外接板支持USB+千兆以太网接口或者其他标准PCIE板卡。
* **新增应用**
  * **文件管理**
    1. 外置存储浏览、文管内访问图库、文件 Picker（选择、后缀过滤、批量授权）。
    2. 路径 Picker 保存文件、打开、分享、重命名、复制、移动、收藏、最近删除、文件属性、列表/宫格视图
 	* **时钟**	新增时钟应用
  	* **计算器**	新增计算器应用
* **预置应用更新**
   * **联系人**	
	 1. 拨号盘搜索及结果快捷操作（详情、黑名单、复制、标记、新建/保存联系人、发短信）
     2. 通话记录（全部/未接）及长按管理（多选、删除、标记、加入黑名单等）
	 3. 联系人搜索、字母索引、智能/自定义群组
	* **短信**	
      1. 短信发送，长短信、表情； 群发、转发、失败重发
      2. 会话列表左滑/长按/滑动多选删除、通知栏整合、标记已读、通知回复、详情页复制、转发、选择文本等操作、 列表与详情展示联系人头像
	* **通话**	
      1. 语音来去电、接听/挂断/拒接、静音、扬声器、音频设备切换、紧急拨号、SOS 连按电源键、紧急位置展示
      2. 紧急联系人及自动求助、来电全屏/横幅、铃声/振动、号码归属地/黄页/标记
	* **设置**	
	  1. 设置内全局搜索、 WLAN / 蓝牙 / 移动网络 / 热点 、壁纸、亮度、深色模式（含定时）、字体与显示大小
	  2. 声音模式、音量面板、来电/信息/通知铃声、通知和状态栏管理、应用管理、锁屏密码、电池、存储

	* **相机**	
		1. 前/后置拍照、前/后置录像、相机 Picker（仅拍照 / 仅录像 / 拍照+录像）、相机设置页、百宝箱入口

	* **桌面**	
		1. 数字密码/滑动解锁、防暴力破解、锁屏时钟与卡片、4× 桌面布局、Dock 栏、角标、快捷方式、编辑模式
		2. 卡片/卡片堆叠/文件夹完整管理、最近任务（锁定、一键清理、滑动删除）
		3. 状态栏、控制中心、手势/三键导航、通知中心（列表、分区、组通知、置顶/静默）
        
# 版本：OpenHarmony 7.0 预计新增包括多模态感知、分布式协同、图形工具链等领域，丰富鸿蒙统一生态底座能力，支撑AI类应用的生态繁荣。 
## 发布日期：2026年H2
### 版本概述
* **数据底座计算与分析:** 端侧AI底座，提供模型部署与运行态管理。
* **智慧数据底座:** RAG框架、语义搜索、智能检索问答能力。
* **多模态融合感知应用:** 口袋防误触等多模态融合感知基础能力。
* **分布式软总线:** P2P增强、星闪SLE连接技术等分布式通信能力。
* **3D图形:** 3D高斯渲染插件，支撑三维渲染场景。
* **智能性能测试分析平台:** 游戏性能采集分析，即插即用，无需ROOT权限。
* **系统安全测试工具集:** DTFuzz引擎与程序分析规则，提升代码安全性。
* **ArkUI生成式UI引擎:** AI模型直接生成UI界面能力。
* **元能力框架:** Agent调用与CLI管理服务，支撑智能体交互。
* **沙箱管理框架:** 动态沙箱隔离，增强系统安全性。
* **系统应用新增:** 桌面增强，设置增强，OOBE、玩机技巧、SIM卡应用、升级、录音机、播放器,浏览器等。