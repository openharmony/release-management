# **OpenHarmony 4.1.4 Release 测试报告**

## **1** **概述**

 本报告针对OpenHarmony 4.1.4 Release 版本的测试验证及质量状态评估，包括对子系统特性和整机专项。 
...
| **版本名称**                      | **测试起始时间** | **测试结束时间** |
| --------------------------------- | ---------------- | ---------------- |
| OpenHarmony 4.1.10.1-4.1.10.5版本 | 2025-3-17        | 2025-5-28        |
...
| 专项性能   | 静态KPI：预期目标胜率90%，实际胜率89.79%(44/49)，不达标 。<br />内存测试 ：整机内存基线841.15M，达标。 | Go with Risk | 详情请见4.4章节           |
...
| -------- | ----------------------------------- | ----------------------------------------------- | ------------------- | ---------------------- | ---------- | ------------ |
| 标准系统 | OpenHarmony应用兼容性测试套         | ACTS  （Application Compatibility Test Suite ） | 看护北向应用兼容性  | 152790                 | 100%       | Go           |
...
| 小型系统 | OpenHarmony应用兼容性测试套         | ACTS  （Application Compatibility Test Suite ） | 看护北向应用兼容性  | liteOS:2890 linux：455 | 100% 100%  | Go           |
...
4.1.10.1-4.1.10.5版本累积测试时间为11315小时 ，APR=7.95(其中蓝牙问题APR=5.74)。遗留12个问题待解决，整体评估带风险发布

| 测试类分类           | 准出标准（APR为牵引值）                                      | 质量状态 | 测试结果                                                     |
| -------------------- | :----------------------------------------------------------- | -------- | ------------------------------------------------------------ |
| 核心服务可靠性       | 1、mediaserver<br/>2、camera_server<br/>3、foundation<br/>4、Samgr<br/>5、hdf_devmgr<br/>6、appspawn<br/>7、softbus_server<br/>8、wifi_manager_se<br/>9、power_host<br/>10、accesstoken_ser<br/>11、render_service<br/> 各核心服务小于标准值0.1 | 不达标   | 版本测试数据：<br/>1、 mediaserver=0.17>0.1(标准值)<br/>     |
| 核心应用可靠性       | 1、com.ohos.mms<br/>2、com.ohos.photos<br/>3、com.ohos.note<br/>4、com.ohos.callui<br/>5、com.ohos.camera<br/>6、com.ohos.contacts<br/>7、com.ohos.settings<br/>8、com.ohos.launcher<br/>9、com.ohos.systemui <br/>10、com.ohos.screenlock<br/>11、com.ohos.filepicker<br/>12、com.example.kikakeyboard <br/>  各核心应用小于标准值0.1 | 不达标   | 版本测试数据：<br/>1、com.ohos.settings=3<br/>2、com.ohos.note=0.61<br/>3、 com.ohos.launcher=0.44<br/> |
| 预置应用异常         | 跟随版本发布预置的应用范围< 3次/千小时                       | 不达标   | 版本测试数据：预制应用APR=7.95                               |
...
| ISSUE  | 描述                                                         |
| ------ | ------------------------------------------------------------ |
| IB9AXC | 进程com.ohos.settings因APP_INPUT_BLOCK出现34次appfreeze      |
| IBM99X | 进程bluetooth_servi下SaInit0线程出现31次cppcrash             |
| IBNOG5 | 进程ohos.samples.distributedcalc因THREAD_BLOCK_6S出现8次appfreeze |
| IC4KPD | 进程com.ohos.settings因THREAD_BLOCK_6S出现4次appfreeze       |
| IBJTWP | 进程com.ohos.launcher因THREAD_BLOCK_6S出现3次appfreeze       |
| IBM9HC | 进程com.ohos.note下的ace.bg.4线程出现3次cppcrash             |
| IBNO9G | 进程com.ohos.launcher下出现2次jscrash                        |
| IB4KHA | 进程Process name:audio_server下线程OS_SocketEvent出现1次cppcrash |
| IBNO2W | 进程media_service下DataReader线程出现1次cppcrash             |
| IBZPRD | 进程netmanager下的OS_IPC_1_472线程出现1次cppcrash            |
| IC1TJO | 进程media_service下DataReader线程出现1次cppcrash             |
| IC1TOL | 进程foundation下OS_FFRT_2_1线程出现1次cppcrash               |

### **4.4** **性能专项测试结论**

准出结论：Go with Risk

静态KPI：预期目标胜率90%，实际胜率89.79%(44/49) ，不达标，整体评估带风险发布。

备注：由于天猫网站无法访问问题，3个测试项无法测试。

内存测试 ：整机内存841.15M，优于基线860M，已达标。

| 应用名称 | 用例名称                                       | 触发方式     | 指标类型 | 安卓基线 | RK3568数据 |
| -------- | ---------------------------------------------- | ------------ | -------- | -------- | ---------- |
| 桌面     | 主桌面静态壁纸滑动                             | 滑动         | 帧率     | 54       | 60         |
| 信息     | 非首次启动短信息                               | 点击（启动） | 完成时延 | 1000     | 375.66     |
| 信息     | 首次启动短信息                                 | 点击（启动） | 完成时延 | 2300     | 1519.44    |
| 相机     | camera启动（冷启动）                           | 点击（启动） | 完成时延 | 2683     | 1286.14    |
| 相机     | camera启动（热启动）                           | 点击（启动） | 完成时延 | 2216     | 566.76     |
| 相机     | 拍照生成缩略图                                 | 点击拍照     | 完成时延 | 1220     | 1330.55    |
| 图库     | 图库缩略图列表滑动                             | 滑动         | 帧率     | 60       | 56.87      |
| 图库     | 图库单个图片缩放                               | 捏合         | 帧率     | 60       | 60         |
| 图库     | 图库缩略图列表滑动                             | 滑动         | 响应时延 | 150      | 115.27     |
| 图库     | 图库单个图片缩放                               | 捏合         | 响应时延 | 150      | 134.72     |
| 图库     | 查看图片                                       | 点击         | 完成时延 | 769      | 543.05     |
| 图库     | 复制一张图片（1张5M图片）                      | 点击         | 完成时延 | 2300     | 543.05     |
| 图库     | 剪切一张图片（1张1M图片）                      | 点击         | 完成时延 | 1833     | 543.05     |
| 图库     | 删除多张图片（2张5M图片）                      | 点击         | 完成时延 | 916      | 929.16     |
| 图库     | 图库从照片切换到相册（Tab切换）                | 点击         | 完成时延 | 442      | 298.61     |
| 图库     | 非首次启动图库                                 | 点击（启动） | 完成时延 | 579      | 375.11     |
| 图库     | 首次启动图库                                   | 点击（启动） | 完成时延 | 1404     | 1522.22    |
| 输入法   | 按键输入速度                                   | 点击         | 响应时延 | 80       | 69.05      |
| 输入法   | 输入法键盘弹出                                 | 点击         | 完成时延 | 540      | 518.05     |
| 设置     | 退出设置                                       | 点击         | 完成时延 | 650      | 213.88     |
| 设置     | 启动设置（原表是冷启动设置）                   | 点击（启动） | 完成时延 | 1145     | 1020.83    |
| 联系人   | 联系人列表滑动                                 | 滑动         | 帧率     | 60       | 55.06      |
| 联系人   | 联系人列表滑动                                 | 滑动         | 响应时延 | 150      | 137.33     |
| 联系人   | 查看一条联系人                                 | 点击         | 完成时延 | 883      | 445.83     |
| 联系人   | 存入联系人                                     | 点击         | 完成时延 | 625      | 329.16     |
| 联系人   | 联系人-拨号子Tab切换                           | 点击         | 完成时延 | 440      | 384.72     |
| 联系人   | 删除联系人                                     | 点击         | 完成时延 | 370      | 229.16     |
| 联系人   | 新建一个联系人                                 | 点击         | 完成时延 | 462      | 341.66     |
| 联系人   | 非首次启动联系人                               | 点击（启动） | 完成时延 | 604      | 372.22     |
| 联系人   | 首次启动联系人                                 | 点击（启动） | 完成时延 | 1270     | 1706.94    |
| webview  | 浏览器打开天猫网页，从上往下滑                 | 滑动         | 帧率     | 54       | block      |
| webview  | 浏览器打开一个纯图片的网页，左右切换图片的滑动 | 滑动         | 帧率     | 54       | 59.8       |
| webview  | 浏览器微博首页滑动                             | 滑动         | 帧率     | 54       | 57.9       |
| webview  | 浏览器天猫界面缩放                             | 捏合         | 帧率     | 54       | block      |
| webview  | 浏览器打开纯文本网页                           | 滑动         | 响应时延 | 150      | 79.16      |
| webview  | 浏览器打开天猫网页                             | 滑动         | 响应时延 | 300      | 73.61      |
| webview  | 浏览器天猫界面滑动                             | 滑动         | 响应时延 | 150      | block      |
| webview  | 浏览器键鼠事件Hover响应时延                    | Hover        | 响应时延 | 200      | 33.3       |
| webview  | 首次启动浏览器                                 | 点击（启动） | 完成时延 | 1621     | 1287.5     |
| SystemUI | 状态栏下拉                                     | 滑动         | 响应时延 | 140      | 113.88     |
| SystemUI | 调起任务管理器                                 | 点击         | 完成时延 | 474      | 163.88     |
| SystemUI | 系统用户界面（清除所有后台进程时间）           | 点击         | 完成时延 | 1000     | 165.75     |
| SystemUI | 后台多任务的切换调度                           | 点击         | 完成时延 | 1041     | 366.67     |
| 开机     | 非首次开机                                     | 重启         | 完成时延 | 22916    | 22366.67   |
| 开机     | 唤醒屏幕                                       |              | 完成时延 | 600      | 37.5       |
| 开机     | 开机                                           | 开机         | 完成时延 | 20512    | 20400.1    |
| 通话     | 拨号盘按键                                     | 点击         | 响应时延 | 88       | 41.67      |
| 时钟     | 首次启动                                       | 点击         | 完成时延 | 704      | 622.27     |
| 计算器   | 首次启动                                       | 点击         | 完成时延 | 1206     | 740.11     |

遗留问题：

| ISSUE  | 描述                                                         |
| ------ | ------------------------------------------------------------ |
| IAE215 | 相机拍照生成缩略图场景基线1220ms，实际值1330.55ms，超基线110.55ms |
| IB0EDJ | 图库缩略图列表滑动基线60帧，实际值56.87帧，差基线3.13帧      |
| IANJ3R | 首次启动图库场景基线1404ms，实际值1522.22ms，超基线118.22ms  |
| IB0EIE | 联系人列表滑动基线60帧，实际值55.06帧，差基线4.94帧          |
| IB0EL5 | 首次启动联系人场景基线1270ms，实际值1706.94ms，超基线436.94ms |
